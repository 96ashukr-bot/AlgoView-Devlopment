...
class Command(BaseCommand):
    help = "Validate a client's broker credentials through the assigned proxy route."
    ...
    def handle(self, *args, **options):
        ...
        if not node.proxy_public_ip_verified:
            raise CommandError("Proxy public IP is not verified.")
        ...
        self.stdout.write(str(adapter.validate_credentials(proxy_config=build_requests_proxy_config(node))))
